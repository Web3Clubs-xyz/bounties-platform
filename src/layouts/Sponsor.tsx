...
import { SelectHackathon, SelectPartner } from '@/features/listing-builder';
import {
  Banner,
  CreateListingModal,
  SponsorInfoModal,
      ...
      <Flex justify="start" display={{ base: 'none', md: 'flex' }} minH="100vh">
        ...
        >
          {session?.user?.role === 'GOD' && (
            <Box px={6} pb={6}>
              {isHackathonRoute ? <SelectHackathon /> : <SelectPartner />}
            </Box>
          )}
          <CreateListingModal isOpen={isOpen} onClose={onClose} />
          <Flex align="center" justify="space-between" px={3} pb={6}>
            {!isHackathonRoute ? (
              <Button
                w="full"
                py={'22px'}
