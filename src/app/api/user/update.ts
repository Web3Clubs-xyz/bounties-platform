...
async function handler(req: NextApiRequestWithUser, res: NextApiResponse) {
  ...
  const { role, skills, currentPartnerId, generateTalentEmailSettings, ...updateAttributes } = req.body;
  let result;
  ...
  try {
    ...
    if (user && user.role === 'GOD' && currentPartnerId) {
      updatedData.currentPartnerId = currentPartnerId;
    }
    ...
    if (generateTalentEmailSettings) {
      ...
      for (const category of categories) {
        await prismaDb.emailSettings.create({
          data: {
    ...
    result = await prismaDb.user.update({
      where: {
        id: userId as string,
      },
      data: updatedData,
